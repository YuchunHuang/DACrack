...
void Conv2d_fft_first(const cv::Mat & image,const  cv::Mat & h,const  cv::Mat  & h2,const cv::Mat & h3, cv::Mat &output, cv::Mat &output2, cv::Mat &output3) {
	...
	addzero(img,img, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	addzero(h,ch, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	addzero(h2,ch2, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	addzero(h3,ch3, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	...
	*/
	...
	dft(ch, ch, 0, size_h_r);
	dft(ch2, ch2, 0, size_h_r);
	dft(ch3, ch3, 0, size_h_r);
	mulSpectrums(img, ch, o, DFT_COMPLEX_OUTPUT);
	mulSpectrums(img, ch2, o2, DFT_COMPLEX_OUTPUT);
	mulSpectrums(img, ch3, o3, DFT_COMPLEX_OUTPUT);
...
void Conv2d_fft_second(const cv::Mat &  image, const cv::Mat & h,const  cv::Mat & h2, cv::Mat &output, cv::Mat &output2) {
	...
	addzero(img,img, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	addzero(h,dh, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	addzero(h2,dh2, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	//h3 = addzero(h3, size_h_r + size_img_r - 1, size_h_c + size_img_c - 1);
	//o = conv2(img, h);//考虑到空间域卷积速度不符合实际要求，改成利用傅里叶变换实现卷积
	/*scut[0] = clock();
	cout << "    图像与卷积核大小处理用时" << (double)(scut[0] - start) / CLOCKS_PER_SEC << "s\n";
	*/
	cv::Mat imgc, dhc, dh2c;
	dft(img, imgc, 0, size_img_r);//只做一次dft，对原图
	dft(dh,dhc, 0, size_h_r);
	dft(dh2, dh2c, 0, size_h_r);
	//dft(h3, h3, 0, size_h_r);
	mulSpectrums(imgc, dhc, o, DFT_COMPLEX_OUTPUT);
	mulSpectrums(imgc, dh2c, o2, DFT_COMPLEX_OUTPUT);
	//mulSpectrums(img, h3, o3, DFT_COMPLEX_OUTPUT);
	cv::Mat oc, o2c;
	dft(o, oc, DFT_INVERSE + DFT_SCALE, o.rows);
	dft(o2, o2c, DFT_INVERSE + DFT_SCALE, o2.rows);
	//dft(o3, o3, DFT_INVERSE + DFT_SCALE, o3.rows);
	/*scut[1] = clock();
	cout << "    fft卷积用时" << (double)(scut[1] - scut[0]) / CLOCKS_PER_SEC << "s\n";*/
	selectpart(oc,oc, floor(size_h_r / 2), (size_img_r + size_h_r - 1) - floor(size_h_r / 2)-1,
		floor(size_h_c / 2), (size_img_c + size_h_c - 1) - floor(size_h_c / 2)-1);
	selectpart(o2c,o2c, floor(size_h_r / 2), (size_img_r + size_h_r - 1) - floor(size_h_r / 2) - 1,
		floor(size_h_c / 2), (size_img_c + size_h_c - 1) - floor(size_h_c / 2) - 1);
	...
	selectpart(oc, oc, floor(size_h_r / 2), (size_image_r + size_h_r - 1) - floor(size_h_r / 2)-1,
		 floor(size_h_c / 2), (size_image_c + size_h_c - 1) - floor(size_h_c / 2)-1);
	selectpart(o2c, o2c, floor(size_h_r / 2), (size_image_r + size_h_r - 1) - floor(size_h_r / 2) - 1,
		floor(size_h_c / 2), (size_image_c + size_h_c - 1) - floor(size_h_c / 2) - 1);
	oc.copyTo(output);
	o2c.copyTo(output2);
	o.release();
...
bool calCurCost(const cv::Mat & src,cv::Mat &dst) {
	cv::Mat temp;
	verticalmeanfilter(src, temp, 10);
	//cv::imwrite("vb10.jpg", temp);
	//////-----------------part1
	////cv::Mat ds;
	//cv::imshow("tmp", temp);
	//cvWaitKey(0);
...
void FeatureCaputure(cv::Mat img, cv::Mat & dst, vector<CArea> &ConArea)
{
	...
	double perlength =sqrt(double(img.rows*img.cols))/1000*150 ;
...
void corpoint_afcur(const cv::Mat &curimg, vector<int>&smcoors) {
	...
	for (int k = 0; k < 2; k++) {
		...
		/*x2 = smcoors[2];
		y2 = smcoors[3];*/
		int l, r, u, d;//设定最大半径为10的窗口范围
		l = x - 10 > 0 ? x - 10 : 0;
		r = x + 10 < c - 1 ? x + 10 : c - 1;
		u = y - 10 > 0 ? y - 10 : 0;
		d = y + 10 < ro - 1 ? y + 10 : ro - 1;
		int maxr = y;
		int maxc = x;
		int maxcur = 0;
		for (int i = l; i <= r; i++) {
